...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // Card/end offsets are captured once (pre-transform) and cached — re-measuring via
    // getBoundingClientRect() here would read back the transform we just applied last
    // frame, creating a feedback loop that drifts and snaps during fast scrolls.
  ...
  useLayoutEffect(() => {
    ...
    const cardList: NodeListOf<HTMLElement> = useWindowScroll
      ? document.querySelectorAll<HTMLElement>('.scroll-stack-card')
      : scroller.querySelectorAll<HTMLElement>('.scroll-stack-card');
    const cards: HTMLElement[] = Array.from(cardList);
    ...
    // Captured once all margins are applied (translateZ(0) is a positional no-op),
    // so later frames measure against a stable, transform-free layout position.
